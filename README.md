...
Fill in the function `check_valid_email`. This function should return the string `valid` if the input string *is a valid email address* and `invalid` if not. We define a valid email as follows:
...
Fill in the function `extract_author_and_book_names`. This function should search an input string for the author and book then return a tuple of them.
...
Replace the word Boy/Girl or boy/girl with Man/Woman or man/woman respectively. The first letter of the replacement word should match the case of the first letter of the original word. Fill in the function `fix_adult_superhero_name` to returns a string with the correct word restored. 
...
Zip `problems.py` and upload the ZIP file to Gradescope.
